...
function updateChart(days, temperatures, city) {
    ...
    weatherChart = new Chart(chartCanvas, {
        ...
        options: {
            ...
            scales: {
                x: {
                    title: { display: true, text: 'Days', color: '#ffffff', font: { size: 20 } },
                    ticks: { color: '#ffffff', font: { size: 20 } },
                    grid: { color: 'rgba(252, 252, 252, 0.5)' }
                },
                y: {
                    title: { display: true, text: 'Temperature (°C)', color: '#ffffff', font: { size: 20 } },
                    ticks: { color: '#ffffff', font: { size: 20 } },
                    grid: { color: 'rgba(252, 252, 252, 0.5)' }
                }
            }
...
async function updateTempVariationChart(city) {
    ...
    tempVariationChart = new Chart(chartCanvas2, {
        ...
        options: {
            ...
            scales: {
                x: { ticks: { color: 'white' }, grid: { color: 'rgba(200, 200, 200, 0.2)' } },
                y: {
                    ticks: { color: 'white' },
...
async function getWeatherAnalysis(city) {
    try {
        const response = await fetch(`http://127.0.0.1:5000/api/analysis?city=${city}`);
